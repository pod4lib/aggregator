class CreateDeltaDumps < ActiveRecord::Migration[8.0]
  def change
    create_table :delta_dumps do |t|
      t.references :stream, null: false, foreign_key: false
      t.references :previous_stream, null: true, foreign_key: false
      t.references :normalized_dump, null: false, foreign_key: true
      t.datetime :published_at

      t.timestamps
    end
    add_index :delta_dumps, :published_at
    ...
    reversible do |dir|
      dir.up do
        say_with_time 'Backfilling full dumps for existing normalized dumps' do
          ...
          NormalizedDump.where.not(full_dump_id: nil).find_each do |normalized_dump|
            DeltaDump.create!(
