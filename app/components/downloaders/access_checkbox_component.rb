...
module Downloaders
  ...
  class AccessCheckboxComponent < ViewComponent::Base
    ...
    def access_entry
      helpers.can?(:control_access, @organization) && @organization.restrict_downloads ? access_link : access_status
    end

    def dom_id
      helpers.dom_id(@resource, :access)
    end

    private

    def access_status
      Downloaders::AccessStatusIconComponent.new(can_access: !@organization.restrict_downloads || access_granted?).call
    end
    ...
    def impacted_organizations
      @impacted_organizations ||= @resource.respond_to?(:organizations) ? @resource.organizations : [@resource]
    end
