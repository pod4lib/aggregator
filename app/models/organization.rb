...
class Organization < ApplicationRecord
  ...
  scope :unrestricted, lambda {
    where.not(id: AllowedConsumer.select(:organization_id).distinct).where(record_access: 'authenticated_users')
  }
  ...
  def allowed_to_consume_organization_ids
    allowed_to_consume.pluck(:id)
  end

  def allowed_to_consume
    (allowed_to_consume_organizations + Organization.unrestricted + groups.flat_map(&:allowed_to_consume_organizations)).uniq
  end
end
