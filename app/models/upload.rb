...
class Upload < ApplicationRecord # rubocop:disable Metrics/ClassLength
  ...
  def update_files_metadata_status
    status =
      if metadata_status_unknown?
        'unknown'
      else
        metadata_status_success? ? 'success' : 'needs_attention'
      end

    update(metadata_status: status)
  end

  def metadata_status_unknown?
    files.any? { |f| f.pod_metadata_status == :unknown }
  end

  def metadata_status_success?
    files.all? { |f| %i[success deletes].include?(f.pod_metadata_status) }
  end
